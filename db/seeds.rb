...
40.times do
  booking = Booking.create!(
    ...
    user_id: Faker::Number.between(from: 2, to: 32),
   ...
   # Create a set to track unique flight_ticket_ids for this booking
   flight_ticket_ids = Set.new

   # Seed TicketInstances for each booking
   rand(1..5).times do
     flight_ticket_id = Faker::Number.between(from: 1, to: 100)

     # Ensure the flight_ticket_id is unique for the given booking
     while flight_ticket_ids.include?(flight_ticket_id)
       flight_ticket_id = Faker::Number.between(from: 1, to: 100)
     end

     flight_ticket_ids.add(flight_ticket_id)
