...
30.times do |n|
  Tour.create!(
    ...
    min_guests: Faker::Number.within(range: 1..10)
    max_guests: Faker::Number.within(range: 11..50)
    deposit_percent: Faker::Number.between(from: 0.0, to: 100.0)
  )
...
100.times do |n|
  ...
  max_uses = Faker::Number.within(range: 10..200)
  Voucher.create!(expiry_date:, code:, min_total_price:, percent_discount:, max_uses:)
end
